...
export default async function(client,interaction,config) {
  if(interaction.isButton()){
    if(interaction.customId.startsWith('apply_')){
       ...
       if(await users_applys_db.has({key:`apply_${interaction.user.id}_${interaction.customId}`}) == true) return await interaction.reply({content:`:x: لقد قمت بالتقديم بالفعل انت!`,ephemeral:true})
